...
const EnhancedChatAgent = () => {
  ...
  const generateLocalResponse = (userMessage: string): string => {
    ...
    if (lowerMessage.includes("skill") || lowerMessage.includes("tech") || lowerMessage.includes("stack")) {
      return `Tijo is a Senior Frontend Engineer with expertise in **${portfolioContext.skills.frontend.join(", ")}**, plus backend skills in **${portfolioContext.skills.backend.join(" and ")}**. He's particularly strong in state management with **${portfolioContext.skills.stateManagement.join(", ")}** and has extensive AWS cloud experience. His testing toolkit includes **${portfolioContext.skills.testing.join(", ")}**.`;
    }
    
    if (lowerMessage.includes("experience") || lowerMessage.includes("work") || lowerMessage.includes("career")) {
      return `Tijo has **${portfolioContext.experience}**, currently working as a Software Engineer at **${portfolioContext.companies[0]}** in ${portfolioContext.location}. He's worked with major companies like **${portfolioContext.companies.join(", ")}** and has delivered projects for enterprise clients including **${portfolioContext.majorClients.join(", ")}**. His career spans from associate trainee to senior engineer, with a strong focus on mentoring and technical leadership.`;
    }
    ...
    if (lowerMessage.includes("infosys") || lowerMessage.includes("home depot") || lowerMessage.includes("ebay")) {
    ...
    if (lowerMessage.includes("project") || lowerMessage.includes("portfolio")) {
      return `Tijo's notable projects include: **${portfolioContext.achievements.slice(0, 3).join("**, **")}**. Each project demonstrates his ability to work across different domains and technologies, with measurable impact like **${portfolioContext.achievements[5]}**.`;
    }
    ...
    if (lowerMessage.includes("aws") || lowerMessage.includes("cloud")) {
      return `Tijo has extensive **AWS experience**, particularly from his time at QBurst Technologies where he **${portfolioContext.achievements[4]}**. He's worked with **${portfolioContext.skills.cloud.join(", ")}**. This cloud expertise complements his frontend skills perfectly.`;
    }
    
    if (lowerMessage.includes("mentor") || lowerMessage.includes("lead") || lowerMessage.includes("team")) {
    ...
    // Default response
    return `That's an interesting question! I'm here to help you learn about **${portfolioContext.name}**, a **${portfolioContext.role}** from **${portfolioContext.location}** with **${portfolioContext.experience}**. He's worked with major companies like **${portfolioContext.companies.slice(0, 3).join(", ")}** and has expertise in modern web technologies. Feel free to ask about his technical skills, work experience, or how to get in touch!`;
  };
              ...
              <div className="flex gap-2">
                ...
                >
                  <div className="flex gap-1 items-center">
                    <span style={{ color: 'var(--theme-accent)' }}>AI thinking</span>
